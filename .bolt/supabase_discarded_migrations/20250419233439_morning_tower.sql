/*
  # Update tenant configuration for freindz.zone
  
  1. Changes
    - Add freindz.zone as a new tenant
    - Configure it to use LYJA Resort settings
    - Ensure proper branding and theme
    
  2. Security
    - Maintains existing RLS policies
*/

-- Insert or update the freindz.zone tenant
INSERT INTO tenants (
  name,
  subdomain,
  settings,
...
-- Copy data from lyja tenant to freindz tenant
DO $$
DECLARE
  lyja_id uuid;
  freindz_id uuid;
BEGIN
  ...
  SELECT id INTO freindz_id FROM tenants WHERE subdomain = 'freindz';
  ...
  SELECT 
    label, start_time, end_time, is_drinks,
    staff_notification_number, freindz_id
  FROM time_slots
  WHERE tenant_id = lyja_id;
